...
impl ToneBank {
    ...
    fn samples_of(&self, index: usize) -> Vec<u16> {
        if self.spec.opaque || self.spec.tag != *b"PATa" {
            return Vec::new();
        }
        self.family[0]
            .records
...
fn rebuild(bank: &ToneBank, keep: &[usize]) -> Result<Raw> {
    // Which sample slots the chosen tones need, in first-seen order.
    ...
    if !slot_map.is_empty() {
        ...
        let waveforms: Vec<&Waveform> = keep_slots
            .iter()
            .map(|&slot| {
                bank.waveforms
                    .iter()
            ...
            .collect::<Result<_>>()?;
        areas.push((*b"USDa", slots.format, build_waveform_area(&waveforms)?));
    }
    ...
    for bank in [a, b] {
        for index in 0..bank.family[0].records.len() {
            ...
            for slot in bank.samples_of(index) {
                ...
                let Some(source) = bank.waveforms.iter().find(|w| w.slot as usize == source_slot)
                else {
                    return Err(Error::Unrecognized(format!(
...
fn build_waveform_area(waveforms: &[&Waveform]) -> Result<Vec<u8>> {
    let info_len = HEADER_LEN + waveforms.len() * DIRECTORY_ENTRY;
    let mut header = [0u8; HEADER_LEN];
    header[0..4].copy_from_slice(&(waveforms.len() as u32).to_le_bytes());
    header[8..12].copy_from_slice(&(info_len as u32).to_le_bytes());
    ...
    for (index, waveform) in waveforms.iter().enumerate() {
        ...
        sections.extend_from_slice(&waveform.bytes);
    }
...
mod tests {
    ...
    fn tone(name: &str, slot: Option<u16>) -> Vec<u8> {
        let mut record = vec![0u8; TONE_LEN];
        record[..16].fill(b' ');
    ...
    fn waveform_body(sections: &[Vec<u8>]) -> Vec<u8> {
        let refs: Vec<Waveform> = sections
            ...
            .collect();
        build_waveform_area(&refs.iter().collect::<Vec<_>>()).unwrap()
    }
